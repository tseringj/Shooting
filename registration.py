...
def homography(cut, img1, img2, mask_img):
    ...
    #cv2.imwrite(os.path.join(global_variables.output_dir, context["uuid"])  + '/matching.png', img3)
    ...
    # if (global_variables.save_extra_stuff):
    #     cv2.imwrite(os.path.join(global_variables.output_dir, context["uuid"])  + '/aligned.jpg', im2Reg)
    if cut:
